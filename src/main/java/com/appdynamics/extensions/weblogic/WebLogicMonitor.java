 ...
 */
public class WebLogicMonitor extends ABaseMonitor {

    private static final Logger logger = ExtensionsLoggerFactory.getLogger(WebLogicMonitor.class);
    ...
    protected void doRun(TasksExecutionServiceProvider taskExecutor) {
        ...
        try {
            ...
            if (config != null) {
                ...
                if (!servers.isEmpty()) {
                    for (Map server : servers) {
                        ...
                        WebLogicMonitorTask task = new WebLogicMonitorTask(taskExecutor.getMetricWriteHelper(), server, getContextConfiguration(), fileSystemLoader);
            ...
            logger.error("WebLogicMonitor Extension can not proceed due to errors in the config.", e);
        }
    }
